...
void parse_irc_line(struct network *net, const char *buffer)
{
	...
	if (data->prefix != NULL)
	{
		...
		{
			log_entry_printf(net,NULL,"T","Servername: %s",data->prefix->servername);
		} else {
			if (data->prefix->nick != NULL)
				log_entry_printf(net,NULL,"T","Nick: %s",data->prefix->nick);
			if (data->prefix->user != NULL)
				log_entry_printf(net,NULL,"T","User: %s",data->prefix->user);
			if (data->prefix->host != NULL)
				log_entry_printf(net,NULL,"T","Host: %s",data->prefix->host);
		}
	}

	log_entry_printf(net, NULL, "T","Command: %s",data->command);

	if (data->c_params != NULL)
		log_entry_printf(net, NULL, "T","Command Parameters: %s",data->c_params_str);

	if (data->rest != NULL)
		log_entry_printf(net, NULL, "T","Rest: %s",data->rest_str);
