...
void irc_loop(void)
{
  ...
  while (1)
  {
    ...
    while (net != NULL)
    {
      ...
      if (net->sock != -1 && net->status != NET_WAITCONNECT)
      {
        numsocks = (net->sock > numsocks) ? net->sock : numsocks;
        FD_SET(net->sock,&socks);
      }

      if (net->status == NET_WAITCONNECT)
      {
        numsocks = (net->sock > numsocks) ? net->sock :numsocks;
        FD_SET(net->sock,&writefds);

        net->status = NET_CHECKERR;
      }
    ...
    while (net != NULL)
    {
      if (net->dcc_listener != -1 && net->dcc_listen_status > DCC_NOTREADY)
      {
      ...
      if (net->sock != -1 && net->status != NET_CHECKERR)
      {
        if (FD_ISSET(net->sock,&socks))
        {
          ...
          if (!irc_in(net))
          {
            /* Network disconnected, try again */
            net->status = NET_DISCONNECTED;
            net->sock   = -1;
            network_connect(net);
          }
