...
void new_join(struct network *net, struct trigger *trig, struct irc_data *data, struct dcc_session *dcc, const char *dccbuf)
{
  ...
  if (!strcmp(net->botnick,data->prefix->nick))
  {
    chan = net->chans;

    while (chan != NULL)
    {
      if (!strcmp(chan->name,data->rest[0]))
        break;
    ...
    if (chan == NULL)
      channel_list_add(&net->chans,new_channel(data->rest[0]));

    chan->status = CHAN_JOINED;
  }
...
void introduce_user(struct network *net, struct trigger *trig, struct irc_data *data, struct dcc_session *dcc, const char *dccbuf)
{
  ...
  else
  {
    ...
    if (!strcmp(user->username,data->prefix->nick))
    {
...
{
  irc_printf(net->sock,"NOTICE %s :\001VERSION Trollbot v%s by poutine\001",data->prefix->nick,VERSION);
