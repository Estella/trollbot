...
void trigger_match(struct network *net, struct irc_data *data)
{
  ...
  {
    /* Eggdrop uses bind SIGN, which loops through the user list of
     * every single channel the bot is in and calls the handler for
     * each channel the bot and user is in, I think that's ugly, so
     * this is a trollbot specific bind, that triggers bind sign 
     * events for scripts which use eggdrop bind SIGN
     * This will clearly be faster than bind sign events
     */
    trig = net->trigs->quit;

    snprintf(newmask,sizeof(newmask),"%s!%s@%s",data->prefix->nick,
                                                data->prefix->user,
                                                data->prefix->host);
    ...
    /* Need to check bind SIGN events */
  }
